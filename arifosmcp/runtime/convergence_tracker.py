...
ConvergenceStates:
  CONVERGED, SOURCE_AHEAD, ARTIFACT_AHEAD, RUNTIME_AHEAD,
  DUPLICATE_INSTALL, MANIFEST_MISMATCH, MODULE_PATH_MISMATCH,
  SCHEMA_DRIFT, TOOL_REGISTRY_DRIFT, VAULT_WRITER_DRIFT,
  UNKNOWN_ARTIFACT, UNREACHABLE
    ...
    def to_receipt(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tool_registry_hash": next(
                (l.observed_value for l in self.layers if l.name == "tool_registry"),
                None,
            ),
            "database_schema_version": next(
                (l.observed_value for l in self.layers if l.name == "database_schema"),
                None,
            ),
            "vault_writer_hash": next(
                (l.observed_value for l in self.layers if l.name == "vault_writer"),
                None,
            ),
...
def probe_artifact_layer() -> ConvergenceLayer:
    dist_info_path = APPROVED_VENV_SITE_PACKAGES
    version = None
    wheel_hash = None
    pkg_sha = None
    for d in Path(dist_info_path).iterdir():
                ...
                for line in record.read_text().splitlines():
                    if line.endswith(".whl,"):
                        ...
                        if len(parts) >= 2 and parts[1].startswith("sha256="):
                            wheel_hash = parts[1]
                            break
            pkg_sha = _sha256_dir(d.parent / "arifosmcp") if (d.parent / "arifosmcp").exists() else None
            break
    observed = {
        "dist_info": Path(dist_info_path).iterdir() and [d.name for d in Path(dist_info_path).iterdir() if d.name.startswith("arifos-") and d.name.endswith(".dist-info")][0],
        "version": version,
    ...
    if version is None or pkg_sha is None:
        return ConvergenceLayer(
            ...
            observed_value=observed, expected_value=expected,
            evidence_ref=f"dir:{dist_info_path}",
...
def probe_installation_layer() -> ConvergenceLayer:
    """Detect duplicate importable distributions."""
    pkg_paths: list[str] = []
    seen = set()
    # Check approved site-packages
    pkg = Path(APPROVED_VENV_SITE_PACKAGES) / "arifosmcp"
    if pkg.exists():
        seen.add(str(pkg.resolve()))
    # Check source tree (must NOT be importable in production)
    src = Path(APPROVED_SOURCE_ROOT) / "arifosmcp"
    if src.exists() and str(src.resolve()) not in seen:
        pkg_paths.append(str(src.resolve()))
    # Check legacy /opt/arifos/app location
    legacy = Path("/opt/arifos/app/arifosmcp")
    if legacy.exists() and str(legacy.resolve()) not in seen:
        pkg_paths.append(str(legacy.resolve()))

    observed = {
        "approved_pkg": str(seen) if seen else None,
        "unauthorized_locations": pkg_paths,
    }
    expected = "one distribution at approved venv site-packages only"
    if pkg_paths:
        return ConvergenceLayer(
            name="installation", state=ConvergenceState.DUPLICATE_INSTALL,
            observed_value=observed, expected_value=expected,
            evidence_ref=f"fs:scan",
...
def probe_import_layer() -> ConvergenceLayer:
    ...
    for mod in key_modules:
        ...
        for root in (APPROVED_VENV_SITE_PACKAGES,):
...
def probe_process_layer() -> ConvergenceLayer:
    ...
    expected = f"executable under /opt/arifos/venv/"
    # Resolve symlink
    real = None
    try:
        ...
        pass
    if real and not real.startswith("/opt/arifos/venv/"):
        return ConvergenceLayer(
...
def probe_tool_registry_layer() -> ConvergenceLayer:
    ...
    try:
        ...
        return ConvergenceLayer(
            ...
            observed_value={"error": str(e)}, expected_value="8 tools, registry hash",
...
def probe_database_schema_layer() -> ConvergenceLayer:
    ...
    expected = "schema version present in alembic"
    if observed["schema_version"] is None:
        return ConvergenceLayer(
            name="database_schema", state=ConvergenceState.SCHEMA_DRIFT,
            observed_value=observed, expected_value=expected,
            evidence_ref=f"dir:{alembic_dir}",
            failure_code="NO_SCHEMA_VERSION",
        )
    return ConvergenceLayer(
...
def _sha256_dir(d: Path) -> str:
    """Compute aggregate sha256 of all .py files in a directory tree."""
    h = hashlib.sha256()
...
def track_convergence(
    *,
    ...
    Args:
        include_service: Required for service-bound actions.
        include_registry: Required before any tool invocation.
        include_database: Required before any DB-touching action.
        include_vault: Required before any seal.

    Returns:
        ConvergenceReport with per-layer state and overall verdict.
    """
    layers: list[ConvergenceLayer] = []
    ...
    if mandatory_failed or required_failed:
        # Find the worst state to use as overall
        non_converged = [l for l in layers if l.state != ConvergenceState.CONVERGED]
        # Priority: UNREACHABLE > MANIFEST_MISMATCH > DUPLICATE_INSTALL > others
        priority = [
    ...
    if artifact_layer:
        ...
        artifact_identity = ArtifactIdentity(
            name="arifosmcp",
            version=ov.get("version") if isinstance(ov, dict) else None,
            sha256=ov.get("package_sha256") if isinstance(ov, dict) else None,
            dist_info_path=str(ov.get("dist_info")) if isinstance(ov, dict) and ov.get("dist_info") else None,
            wheel_hash=ov.get("wheel_hash") if isinstance(ov, dict) else None,
        )
    else:
        artifact_identity = ArtifactIdentity(
    ...
    if service_layer and isinstance(service_layer.observed_value, dict):
        ...
        service_identity = ServiceIdentity(
            ...
            started_at=None,  # could compute from /proc but skip for now
        )
    else:
        service_identity = ServiceIdentity(
