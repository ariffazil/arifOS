...
# New canonical compiled registry (generated by aaa-registry compile)
_COMPILED_REGISTRY = Path("/root/AAA/registry/compiled")
_COMPILED_FEDERATION = _COMPILED_REGISTRY / "FEDERATION_MODEL.json"
...
class ModelShadowLoader:
    ...
    def load_all(self) -> None:
        ...
        # Try new canonical compiled registry first
        if _COMPILED_FEDERATION.exists():
            # The compiled registry currently contains shadow profiles only;
            # routing constraints remain canonical in the legacy federation file.
    ...
    def _load_compiled_registry(self) -> None:
        """Load from the new canonical compiled registry."""
        if not _COMPILED_FEDERATION.exists():
            return
        try:
            with open(_COMPILED_FEDERATION) as f:
                data = json.load(f)
            for model in data.get("models", []):
                ...
                profile = ShadowProfile(
                    ...
                    shadows=shadows,
                    floor_posture=model.get("floor_deltas", {}),
                    sources=[],
                    lifecycle={},
                )
