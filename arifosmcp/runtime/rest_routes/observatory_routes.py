...
def _organs_block(mcp: Any) -> dict[str, dict[str, Any]]:
    ...
    for name, label, host, port in organs:
        ...
        out[name] = {
            ...
            "evidence": _pf(
                "loaded" if dp["capability"] else "unavailable",
                source=f"{label} domain evidence",
                state="observed" if dp["capability"] else "unknown",
                confidence=0.7 if dp["capability"] else 0.0,
                observation_method=_OBS_METHOD_DERIVED,
                independent=True,
            ),
            "governance": _pf(
                "DELEGATES_TO_KERNEL" if name != "arifos" else "active",
            ...
            "last_receipt": _pf(
                "pending" if dp["capability"] else "unavailable",
                source=f"{label} seal_chain tail",
                state="derived" if dp["capability"] else "unknown",
                confidence=0.7 if dp["capability"] else 0.0,
                observation_method=_OBS_METHOD_DERIVED,
                independent=True,
            ),
