...
class RasaDuniaSignal(BaseModel):
    ...
    asset: str = Field(
        ..., description="The asset identifier (gold, oil, gas, copper, fx, lng_europe)"
    )
    ...
    risk_band: RasaDuniaRiskBand = Field(
        RasaDuniaRiskBand.SAFE, description="Calculated stress level"
    )
...
class RasaDuniaEnvelope(BaseModel):
    ...
    verdict: RasaDuniaVerdict = Field(
        RasaDuniaVerdict.PROCEED, description="Somatic verdict for tool execution"
    )
    entropy_index: float = Field(
        ..., description="Calculated world-state entropy change (dS_dunia)"
    )
    weakest_vector: str = Field(..., description="The signal vector carrying the highest stress")
    signals: dict[str, RasaDuniaSignal] = Field(
        default_factory=dict, description="Active signal mapping"
    )
...
class RasaDuniaEngine:
    ...
    def _calculate_risk(
        self, key: str, value: float, baseline: float
    ) -> tuple[RasaDuniaRiskBand, float]:
    ...
    def evaluate(self, inputs: dict[str, float]) -> RasaDuniaEnvelope:
        ...
        for key, val in inputs.items():
            ...
            unit = "pct"
            if vector == RasaDuniaVector.THERMAL:
                unit = "C"
            elif vector == RasaDuniaVector.PRESSURE:
                unit = "bar"
            elif vector == RasaDuniaVector.FRICTION:
                unit = "hours"
            elif vector == RasaDuniaVector.DEPLETION:
                unit = "Bscf"
            elif vector == RasaDuniaVector.DECAY:
                unit = "ratio" if asset == "gold" else "MYR"
        ...
        if weakest_signal.risk_band == RasaDuniaRiskBand.CRISIS:
            ...
            notes.append(
                f"HOLD triggered: Crisis risk band detected on physical vector {weakest_key} (val: {weakest_signal.value} {weakest_signal.unit})"
            )
        elif weakest_signal.risk_band == RasaDuniaRiskBand.STRAIN:
            verdict = RasaDuniaVerdict.SABAR
            notes.append(
                f"SABAR triggered: Elevated physical strain detected on vector {weakest_key} (val: {weakest_signal.value} {weakest_signal.unit})"
            )
        else:
            verdict = RasaDuniaVerdict.PROCEED
