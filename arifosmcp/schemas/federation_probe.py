...
class OrganProbeResult(BaseModel):
    ...
    health: Literal["ok", "degraded", "down"] = Field(
        "down", description="Health status of the organ"
    )
    ...
    evidence: str = Field(
        "direct_probe", description="Type of evidence gathered (e.g. direct_probe)"
    )
...
class FederationProbeResult(BaseModel):
    ...
    federation: list[OrganProbeResult] = Field(
        default_factory=list, description="Probed status of all organs"
    )
    verdict: Literal["PROCEED", "HOLD", "VOID"] = Field(
        "HOLD", description="Substrate level operational verdict"
    )
