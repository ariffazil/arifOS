...
@pytest.mark.xfail(
    strict=True,
    reason="T1-01: arifOS kernel must never SEAL its own actions — requires external Gödel-lock witness (T3 F13)",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="T1-02: A-FORGE must never issue SEAL/HOLD/SABAR/VOID verdicts — executor ≠ judge (T3 F13)",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="T1-03: Anonymous actors must never mutate — requires session_id enforcement at all MUTATE-class tools (T2)",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="T1-04: Session tokens must be validated at every organ boundary — cross-organ session chain integrity (T2)",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="T1-05: VAULT999 must reject any write that does not flow through arif_seal chain (T2)",
)
...
@pytest.mark.xfail(
    strict=True, reason="T1-06: WELL must never emit diagnostic claims — REFLECT_ONLY doctrine (T2)"
)
...
@pytest.mark.xfail(
    strict=True, reason="T1-07: GEOX must never emit verdicts — computes evidence only (T2)"
)
...
@pytest.mark.xfail(
    strict=True, reason="T1-08: WEALTH must never allocate — computes capital metrics only (T2)"
)
...
@pytest.mark.xfail(
    strict=True,
    reason="T1-09: No execution path may bypass FloorEnforcer — requires enum of all execution paths + gate (T2)",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="T1-10: No direct VAULT999 write outside arif_seal — requires filesystem guard (T2)",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="T1-11: Cooling ledger records must never be deleted or rewritten — append-only like VAULT999 (T2)",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="T1-12: No organ may claim another organ's identity_hash — requires identity attestation at bridge (T2)",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="T1-13: Seal chain must never have a broken hash link — requires chain integrity verifier at every seal (T2)",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="T1-14: Self-modification must pass cooling gate with external witness before application (T3 F13)",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="E4: Stale WELL biometrics must trigger IRREVERSIBLE HOLD — freshness gate not yet enforced at kernel level (T2)",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="E5: forge_execute_sealed without external_witness_ref must HOLD — Gödel lock enforcement (T3 F13)",
)
