...
Goal: arif_memory_recall(mode='store') for durable/tier='L4' or higher
      requires f13_severity_acknowledged=True or returns HOLD.

Pass criteria:
    - mode='store', tier='L4', f13_severity_acknowledged=False → HOLD
    - mode='store', tier='L4', f13_severity_acknowledged=True → SEAL (with provenance)
...
Current status (2026-06-12): UNKNOWN (depends on memory_recall mode dispatch).
    ...
    verifies the f13_severity_acknowledged gate exists.
"""
...
def test_memory_store_l4_requires_ack():
    """L4 durable store must require f13_severity_acknowledged."""
