"""
test_abcd_apex.py — ABCD Agentic Test for APEX Theory + Capability Governance
==============================================================================

17 tests across 4 classes. All must pass before SEAL.
...
from arifosmcp.runtime.public_surface import CANONICAL_9, EXPANDED_45
...
def run_all() -> bool:
    ...
    # B1: nine_signal APEX-derived (not status labels)
    ns = _nine_signal_from_apex(G=0.85, C_dark=0.05, system_health=0.95)
    assert ns["omega"]["computed"] is True
    ...
    assert s.saksi_required is True
    assert s.malu_delta == 0.08
    ...
    assert h.state == HantarState.SESAT
    assert h.tebus.required is True
    assert h.tebus.saksi_required is True
    results["B3_hantar"] = "PASS"

    # B4: MALU persists to disk
    ms = get_malu_score("test-abcd")
    ms.record_adat_violation("ADAT-01-KEJUJURAN", context={"test": "abcd"})
    assert _PERSIST_FILE.exists()
    persisted = json.loads(_PERSIST_FILE.read_text())
    assert "test-abcd" in persisted
    ...
    # D1: Every EXPOSED tool exists in capability graph
    exposed = {n for n, e in CANONICAL_TOOLS.items() if e.get("expose", True) is not False}
    assert len(exposed - set(EXPANDED_45)) == 0
    results["D1_registry"] = "PASS"

    # D2: Deprecated tools correctly marked
    deprecated = {n for n, e in CANONICAL_TOOLS.items() if e.get("expose", True) is False}
    assert len(deprecated) >= 8
    results["D2_deprecated"] = "PASS"

    # D3: Unknown → SESAT with JALAN_ALAT
    s2 = emit_sesat(
        source_node="k",
        failure_code="JALAN_ALAT",
        failed_claim="tool exists",
        observed_reality="Unknown",
    )
    assert s2.failure_code == FailureCode.JALAN_ALAT
    results["D3_unknown_sesat"] = "PASS"

    # D4: nine_signal includes traceable primitive inputs
    ns2 = _nine_signal_from_apex(G=0.75, C_dark=0.15, system_health=0.90)
    assert all(k in ns2["omega"] for k in ["G", "C_dark", "formula", "computed"])
    results["D4_traceable"] = "PASS"

    # ═══ SUMMARY ═══
    total = len(results)
    passed = sum(1 for v in results.values() if v == "PASS")
    print(f"═══ ABCD AGENTIC TEST ═══")
...
if __name__ == "__main__":
    ok = run_all()
    if ok:
        print(f"\n🟢 ALL {len(results)} TESTS PASS — SEAL READY")
        sys.exit(0)
    else:
        print(f"\n🔴 FAILED")
        sys.exit(1)
