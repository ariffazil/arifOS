...
FORGE_DIR = Path("/root/forge_work/auto-remediate"); FORGE_DIR.mkdir(parents=True, exist_ok=True)
...
def execute(f):
    ...
    if c == "topology":
        subprocess.run(["python3","/root/arifOS/scripts/federation_reality_probe.py","--write-json"], capture_output=True, timeout=30)
        return True, f"{fid}: re-ran reality probe (refresh edge cache)"

    if c == "provenance":
        subprocess.run(["bash","-c","cd /root/arifOS && rsync -aq --delete --exclude='.git' arifosmcp/ /opt/arifos/app/arifosmcp/ && git rev-parse HEAD > /opt/arifos/app/.git_commit && systemctl restart arifos"], capture_output=True, timeout=60)
        return True, f"{fid}: synced source→deployed + restarted"

    if c == "identity":
        subprocess.run(["python3","/root/arifOS/scripts/emit_observatory_snapshot.py"], capture_output=True, timeout=30)
        return True, f"{fid}: re-ran observatory emitter (refresh organ identity)"

    if c == "receipt":
        return True, f"{fid}: VAULT gaps — sovereign non-issue (SOVEREIGN-2026-07-18-001). No action."
...
def verify():
    r = subprocess.run(["python3","/root/arifOS/scripts/emit_observatory_snapshot.py"], capture_output=True, timeout=45)
    return r.returncode == 0

def seal(fid, ok, note):
    entry = {"actor":"auto-remediate","verdict":"SEAL" if ok else "HOLD", "finding":fid, "note":note, "ts":_now()}
    with open(FORGE_DIR / f"{fid}-{int(time.time())}.json","w") as f: json.dump(entry,f,indent=2)
    with open(VAULT,"a") as f: f.write(json.dumps(entry)+"\n")
