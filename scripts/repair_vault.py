...
from VAULT999.seal_law import GENESIS_CHAIN_HASH, GENESIS_ENTRY_HASH, build_entry
...
def repair(jsonl_path):
    ...
    for i, e in enumerate(entries):
        ...
        for k, v in e.items():
            if k not in new_entry and k not in [
                "chain",
                "chain_hash",
                "seal_hash",
                "entry_hash",
        ...
        if (
            e.get("seal_hash") != new_entry["seal_hash"]
            or e.get("chain_hash") != new_entry["chain_hash"]
            or e.get("entry_hash") != new_entry["entry_hash"]
        ):
            fixed_count += 1
